	...
	{		
		...
		private function readIFD(ifd:int):void{
			...
			for each(var jumpTarget:Object in jumps)
			{
				...
				for(;m_loc<jumpTarget.address;++m_loc)
				{
					if(m_data.bytesAvailable + m_data.position < m_data.length)
					{
						m_data.readByte();
					}
					else
			...
			// read the next IFD
			if(nextIFD && nextIFD < m_data.bytesAvailable)
